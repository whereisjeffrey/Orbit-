...
class PracticeConversationService {
    ...
    func getSolResponse(
        ...
        { \
          ...
          "native_correction": "ALWAYS provide this — rewrite the user's message as a native would say it. FORMAT DEPENDS ON TYPE: For simple swaps (gender, conjugation, preposition) — show ONLY the changed fragment: 'a prédio → o prédio'. For structural changes, idioms, or complete rephrasing — show the full corrected sentence. NEVER put YOUR response here. null ONLY if their message was already perfect native-level.", \
