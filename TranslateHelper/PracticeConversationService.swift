...
class PracticeConversationService {
    ...
    func getSolResponse(
        ...
        { \
          ...
          "native_correction": "Show what you corrected in THE USER'S message. FORMAT DEPENDS ON TYPE: For simple swaps (gender, conjugation, preposition) — show ONLY the changed fragment: 'a prédio → o prédio'. For structural changes, idioms, or complete rephrasing where context matters — show the full corrected sentence so the user can see how it restructures. Use your judgment: if a 3-word swap captures it, keep it short. If the whole sentence changed shape, show the whole thing. NEVER put YOUR response here. null only if perfect.", \
          "native_correction_notes": "1-2 sentences max. Why you changed it — the rule, the pattern, the nuance. Use \(langName) words inline. null if native_correction is null.", \
